...
public class TaskSeriesCollection extends AbstractSeriesDataset
    ...
    public Number getEndPredValue(int row, int column, int depencyInterval) {
		Comparable rowKey = getRowKey(row);
		Comparable columnKey = getColumnKey(column);
		return getEndPredValue(rowKey, columnKey, depencyInterval);
	}
    
	public Number getEndPredValue(Comparable rowKey, Comparable columnKey,
			int depencyInterval) {
		Number result = null;
		int row = getRowIndex(rowKey);
		TaskSeries series = (TaskSeries) this.data.get(row);
		Task task = series.get(columnKey.toString());
		if (task != null) {
			Task dep = task.getPredecessor(depencyInterval);
			if (dep != null) {
				TimePeriod duration = dep.getDuration();
				result = new Long(duration.getEnd().getTime());
			}
		}
		return result;
    ...
    public int getPredColumnIndex(int row, int column,
			int depencyInterval){
		int result=0;
		Comparable rowKey = getRowKey(row);
		Comparable columnKey = getColumnKey(column);
		...
		if (task != null) {
			Task dep = task.getPredecessor(depencyInterval);
			if (dep != null) {
				result = getColumnIndex(dep.getDescription());
			}
		}
		return result;
	}

    public int getSubColumnIndex(int row, int column,
			int subInterval){
		int result=0;
		Comparable rowKey = getRowKey(row);
		Comparable columnKey = getColumnKey(column);
		...
		if (task != null) {
			Task sub = task.getSubtask(subInterval);
			if (sub != null) {
				result = getColumnIndex(sub.getDescription());
			}
		}
		return result;
	}
    
	public Number getStartPredValue(int row, int column, int depencyInterval) {
		Comparable rowKey = getRowKey(row);
		Comparable columnKey = getColumnKey(column);
		return getStartPredValue(rowKey, columnKey, depencyInterval);
	}
	
	public Number getStartPredValue(Comparable rowKey, Comparable columnKey,
			int depencyInterval) {
		Number result = null;
		int row = getRowIndex(rowKey);
		TaskSeries series = (TaskSeries) this.data.get(row);
	...
	public int getPredIntervalCount(int row, int column) {
		Comparable rowKey = getRowKey(row);
		Comparable columnKey = getColumnKey(column);
		return getPredIntervalCount(rowKey, columnKey);
	}
	
	
	public int getPredIntervalCount(Comparable rowKey,
		...
		Task task = series.get(columnKey.toString());
		if (task != null) {
			result = task.getPredecessorCount();
		}
		return result;
	}
    
	public boolean getMilestone(int row, int column){
		TaskSeries series = (TaskSeries) this.data.get(row);
		Comparable columnKey = getColumnKey(column);
		Task task = series.get(columnKey.toString());
		boolean result=false;
		if (task != null) {
			result = task.isMilestone();
		}
		return result;
	}
	
	public int getResourceCount(int row, int column){
		TaskSeries series = (TaskSeries) this.data.get(row);
		Comparable columnKey = getColumnKey(column);
		Task task = series.get(columnKey.toString());
		return task.getRecursoCount();
	}

	public String getResource(int row, int column, int resourceIndex){
		TaskSeries series = (TaskSeries) this.data.get(row);
		Comparable columnKey = getColumnKey(column);
		Task task = series.get(columnKey.toString());
		return task.getRecurso(resourceIndex);
	}

	public Task getTask(int row, Comparable taskkey){
		TaskSeries series = (TaskSeries) this.data.get(row);
		Task task = series.get(taskkey.toString());
		return task;
	}
}